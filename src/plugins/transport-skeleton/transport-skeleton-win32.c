...
static bool
setup_io_slot(struct io_slot *slot, struct transport_skeleton_context *ctx,
...
destroy_io_slot(struct io_slot *slot)
{
    if (slot->status == IO_SLOT_PENDING)
    {
        ...
        {
            transport_skeleton_log(slot->ctx, PLOG_ERR,
...
struct transport_skeleton_socket_win32
{
...
struct openvpn_vsocket_vtab transport_skeleton_socket_vtab;

static void
free_socket(struct transport_skeleton_socket_win32 *sock)
{
    ...
    */
    ...
    if (!can_free)
    {
        ...
        transport_skeleton_log(sock->ctx, PLOG_ERR, "doomed, leaking the entire socket structure");
        return;
    }
...
transport_skeleton_win32_bind(void *plugin_handle,
                     const struct sockaddr *addr, openvpn_vsocket_socklen_t len)
{
    struct transport_skeleton_socket_win32 *sock = NULL;
...
//    transport_skeleton_munge_addr(addr_rev, len);

    sock = calloc(1, sizeof(struct transport_skeleton_socket_win32));
    if (!sock)
        goto error;
...
error:
    transport_skeleton_log((struct transport_skeleton_context *) plugin_handle, PLOG_ERR,
...
static void
ensure_pending_read(struct transport_skeleton_socket_win32 *sock)
{
...
static bool
complete_pending_read(struct transport_skeleton_socket_win32 *sock)
{
...
static void
consumed_pending_read(struct transport_skeleton_socket_win32 *sock)
{
    struct io_slot *slot = &sock->slot_read;
...
static inline bool
complete_pending_write(struct transport_skeleton_socket_win32 *sock)
{
...
static void
transport_skeleton_win32_request_event(openvpn_vsocket_handle_t handle,
                              openvpn_vsocket_event_set_handle_t event_set, unsigned rwflags)
{
    ...
    transport_skeleton_log(sock->ctx, PLOG_DEBUG, "request-event: %d", rwflags);
    sock->last_rwflags = 0;
...
static bool
transport_skeleton_win32_update_event(openvpn_vsocket_handle_t handle, void *arg, unsigned rwflags)
{
    transport_skeleton_log(((struct transport_skeleton_socket_win32 *) handle)->ctx, PLOG_DEBUG,
                  "update-event: %p, %p, %d", handle, arg, rwflags);
    if (arg != handle)
        return false;
    ((struct transport_skeleton_socket_win32 *) handle)->last_rwflags |= rwflags;
    return true;
}

static unsigned
transport_skeleton_win32_pump(openvpn_vsocket_handle_t handle)
{
    struct transport_skeleton_socket_win32 *sock = (struct transport_skeleton_socket_win32 *)handle;
    unsigned result = 0;
    ...
    transport_skeleton_log(sock->ctx, PLOG_DEBUG, "pump -> %d", result);
    return result;
}
...
//    if (addr_copy_len > 0)
//        transport_skeleton_munge_addr(addr, addr_copy_len);
...
static void
transport_skeleton_win32_close(openvpn_vsocket_handle_t handle)
{
    free_socket((struct transport_skeleton_socket_win32 *) handle);
}

void
transport_skeleton_initialize_socket_vtab(void)
{
    transport_skeleton_socket_vtab.bind = transport_skeleton_win32_bind;
    transport_skeleton_socket_vtab.request_event = transport_skeleton_win32_request_event;
    transport_skeleton_socket_vtab.update_event = transport_skeleton_win32_update_event;
    transport_skeleton_socket_vtab.pump = transport_skeleton_win32_pump;
    transport_skeleton_socket_vtab.recvfrom = transport_skeleton_win32_recvfrom;
    transport_skeleton_socket_vtab.sendto = transport_skeleton_win32_sendto;
    transport_skeleton_socket_vtab.close = transport_skeleton_win32_close;
}
