...
/* Reset the seek position for FILE to the beginning. It normally
   starts at the beginning when the file is opened, but may be changed
   by most read operations. If an update procedure on FILE has
   completed, further calls to u2fdbt_next will use the new version of
   the file.

   This is like Unix 'setpwent' called while the file is open.
*/
int u2fdbt_rewind(struct u2fdbt_File *file);

/* Parse and return the 'next' record in FILE, advancing the seek
   position. Returns NULL on error or end of file. If an update
   procedure on FILE completes between calls to u2fdbt_next, this
   function may either detect this and return an error, or continue
   returning records from the original version of FILE.

   This is like Unix 'getpwent'.

   If this function is called during an update procedure, it must be
   followed by a call to an appropriate update function before the
   next call to u2fdbt_next. See u2fdbt_begin_update.
*/
const struct u2fdbt_Record *u2fdbt_next(struct u2fdbt_File *file);
...
*/
const struct u2fdbt_Record *u2fdbt_find(
        struct u2fdbt_File *file, const char *name);
...
/* During an update procedure, indicate that the last record read from
   FILE using u2fdbt_next should be kept as-is. See
   u2fdbt_begin_update.

   This does not invalidate the pointer returned by u2fdbt_next.
*/
int u2fdbt_keep_last(struct u2fdbt_File *file);

/* During an update procedure, indicate that the last record read from
   FILE using u2fdbt_next should be replaced with NEW_RECORD. See
   u2fdbt_begin_update. 

   NEW_RECORD will be copied as needed and is not required to remain
   valid beyond the duration of this call.
*/
int u2fdbt_replace_last(
        struct u2fdbt_File *file,
        const struct u2fdbt_Record *new_record);
...
/* During an update procedure, indicate that NEW_RECORD should be
   ...
   valid beyond the duration of this call.
*/
int u2fdbt_insert_new(const struct u2fdbt_Record *new_record);

/* During an update procedure, indicate that the last record read from
   FILE using u2fdbt_next should be deleted and not included in the
   updated file. See u2fdbt_begin_update.

   This does not invalidate the pointer returned by u2fdbt_next.
*/
int u2fdbt_delete_last(struct u2fdbt_File *file);

/* Before finishing an update procedure when the seek pointer is not
   at end of file, indicate that all further records from FILE should
   be kept as-is. The seek pointer is set to the end of the file. See
   u2fdbt_begin_update.
*/
int u2fdbt_keep_rest(struct u2fdbt_File *file);

/* Before finishing an update procedure when the seek pointer is not
   at end of file, indicate that all further records from FILE should
   be deleted. The seek pointer is set to the end of the file. See
   u2fdbt_begin_update.
*/
int u2fdbt_delete_rest(struct u2fdbt_File *file);
